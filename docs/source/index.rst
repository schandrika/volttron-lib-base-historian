...
Historian Framework
===================
Historian agents automatically collect data from a subset of topics on the message bus and store them in a data
store of choice. They provide interfaces to query this data based on different criteria.
...
Base Historian
 ...
 1. subscribes to the configured set of topics on the message bus
 2. caches all received messages to a local database before publishing it to the Historian.  This
    allows recovery from unexpected happenings before the successful writing of data to the Historian.
 3. takes care of parsing and validating common configurations that the user provides
 4. can optionally rename topics before persisting it. This is useful when topic names have sensitive
...
By default the base historian subscribes to 4 separate root topics:
...
Each root topic has a :ref:`specific message syntax <Historian-Topic-Syntax>` that is expected for incoming data.
