...
def test_base_historian_agent_should_filter_duplicates(base_historian_agent):
    ...
    # Because the _id is ever increasing we remove them from the lists
    for x in expected_to_publish_list:
        x.pop("_id")
    for x in base_historian_agent.last_to_publish_list:
        x.pop("_id")
    assert expected_to_publish_list == base_historian_agent.last_to_publish_list
